    ...
    public partial class MainForms : Form
    {
        ...
        private bool relayStateSend()
        {
            ...
            if (NetDebugForm.hasOpen)
            {
                ...
                byte temp = 0x00;
                if (button12.Text=="关闭") temp += 0x01;
                if (button17.Text == "关闭") temp += 0x02;
                if (button15.Text == "关闭") temp += 0x04;
                if (button18.Text == "关闭") temp += 0x08;
                if (button16.Text == "关闭") temp += 0x10;
                if (button19.Text == "关闭") temp += 0x20;
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            if (NetDebugForm.hasOpen)
            {
                ...
                int month = starTime.Month;
                int day = starTime.Day;
                int hour = starTime.Hour;
                int minute = starTime.Minute;
                int second = starTime.Second;
                sendbytes[0] = 0x2e;
                sendbytes[1] = 0x2e;
                sendbytes[2] = 0x00;
                sendbytes[3] = 0x15;
        ...
        private void setControlsAvailability(bool avail)
        {
            ...
            button17.Enabled = avail;
            button18.Enabled = avail;
            button19.Enabled = avail;
        ...
        {
            if (button10.Text == "打开" && MessageBox.Show("确定是要开启所有该继电器吗？", "询问", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                relayStateAutoSend(true); 
                button10.Text = "关闭";
            }
            else if (button10.Text == "关闭"&&MessageBox.Show("确定是要关闭所有继电器吗？", "询问", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (NetDebugForm.hasOpen)
            {
                ...
                String tempDay= int.Parse(textBox65.Text).ToString("0000");
                MessageBox.Show(textBox65.Text);
                String tempTime = textBox66.Text.Replace(":", "");
                
                DateTime timeMatch;
                if (false == DateTime.TryParse(textBox66.Text, out timeMatch))
                {
                    MessageBox.Show("时间格式不正确！");
                    return;
                }
                char[] temp = (tempDay + tempTime).ToCharArray();
